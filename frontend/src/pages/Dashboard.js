  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (isMentor) {
        ...
        setRequests(requestsRes.data.requests.filter(r => r.status === 'pending').slice(0, 5));
      }
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            ...
            {isMentor && profile?.verification_status === 'verified' && 'Help students achieve their career goals'}
            {isMentor && profile?.verification_status === 'pending' && 'Your mentor profile is pending verification'}
          </p>
        </div>
        ...
        {isMentor && profile?.verification_status === 'pending' && (
          <div className="mb-6 bg-yellow-50 border border-yellow-400 text-yellow-700 px-4 py-3 rounded">
            Your mentor profile is under review. You'll be notified once it's verified.
          </div>
        )}
          ...
          {isStudent && (
            <>
              <div className="bg-white p-6 rounded-lg shadow">
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Career Assessment</h3>
          ...
          {isMentor && (
            <>
              <div className="bg-white p-6 rounded-lg shadow">
                ...
                <p className="text-3xl font-bold text-primary-600 mb-4">{requests.length}</p>
                <Link
                  to="/requests"
                  className="text-primary-600 hover:text-primary-700 font-medium"
                >
              ...
              <div className="bg-white p-6 rounded-lg shadow">
                ...
                <p className="text-gray-600 mb-4">Share knowledge with students</p>
                <Link
                  to="/resources"
                  className="text-primary-600 hover:text-primary-700 font-medium"
                >
              ...
              <div className="bg-white p-6 rounded-lg shadow">
                ...
                <p className="text-3xl font-bold text-primary-600 mb-4">{sessions.length}</p>
                <Link
                  to="/sessions"
                  className="text-primary-600 hover:text-primary-700 font-medium"
                >
                ...
                <div className="space-y-4">
                  {messages.map((message) => (
                    <div key={message.id} className="border-l-4 border-primary-500 pl-4">
                      <p className="font-medium text-gray-900">
                        {isStudent ? message.receiver_name : message.sender_name}
                      </p>
              ...
              <div className="space-y-4">
                {assessments.map((assessment) => (
                  <div key={assessment.id} className="border-l-4 border-green-500 pl-4">
                    <p className="font-medium text-gray-900">Assessment {assessment.assessment_id}</p>
                    <p className="text-sm text-gray-600">
